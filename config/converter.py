...
class ConfigSpaceConverter(BaseConverter):
    ...
    def convert(self, config: MetaConfigCollection) -> Dict[str, ConfigurationSpace]:
        ...
        for key, estimator in config.items():
            estimator_cs = self.__get_algo_config(estimator)
    ...
    @staticmethod
    def __get_algo_config(estimator: MetaConfig) -> ConfigurationSpace:
        """
...
class HyperoptConverter(BaseConverter):
    ...
    def convert(self, config: MetaConfigCollection) -> hp.choice:
        ...
        """
        config_space = []
        for key, algo in config.items():
            d = {'type': key}
            graph = ConfigInheritanceGraph(algo)
...
class RandomSearchConverter(BaseConverter):
    ...
    def convert(self, config: MetaConfigCollection):
        ...
        for name, algo in config.items():
            ...
            for key, value in algo.items():
                ...
                d.update({key: self.__get_algo_config(key, value, graph)})
            estimators.update({name: d})
        return estimators

    def __get_algo_config(self, key, value: ConfigFeature, graph: ConfigInheritanceGraph):
        ...
        if value.type == UNI_INT:
            ...
            for choice in value.choices:
                ...
                if (len(children) == 1):
                    ...
                    choices_list.append(self.__get_algo_config(children[0][1], c, graph))
                else:
